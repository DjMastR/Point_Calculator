...
erettsegi_nevek = ["Magyar", "Történelem", "Matematika", inyelv, val_e]

e1 = Erettsegi(erettsegi_nevek[0])
e2 = Erettsegi(erettsegi_nevek[1])
e3 = Erettsegi(erettsegi_nevek[2])
e4 = Erettsegi(erettsegi_nevek[3])
e5 = Erettsegi(erettsegi_nevek[4])
...
emelt_szam = None
while not e:
    emelt_szam = input("Emeltek száma: ")
    try:
        while int(emelt_szam) < 0 or int(emelt_szam) > 5:
            print("Helytelen adat")
            emelt_szam = input("Kérlek írd be mégegyszer: ")
        e = True
    except ValueError as hiba:
        print("Hibás adat")

emeltek = []
for sorszam in range(int(emelt_szam)):
    emelt_nev = ""
    emelt_nev = szoveg_ellenorzo(emelt_nev, f"Mi volt az/a {sorszam+1}. érettségid? ")
    t = False
    while not t:
        if emelt_nev in erettsegi_nevek:
            emeltek.append(emelt_nev)
            t = True
        else:
            print("Nincs ilyen érettségi tantárgyad")
            emelt_nev = szoveg_ellenorzo(emelt_nev, f"Mi volt az/a {sorszam+1}. érettségid? ")

emelt = 0
for tantargy in emeltek:
    szazalek = e_list[erettsegi_nevek.index(tantargy)]
    if int(szazalek.szazalek) >= 45:
        emelt += 1
